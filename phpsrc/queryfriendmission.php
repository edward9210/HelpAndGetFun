...
<?php
      ...
      $sel=mysql_query("select tmp.*,user.headimg from ((select * from cre_par,mission where mission.missionname=cre_par.mission and mission.missionstate<>2 and cre_par.cr_or_pr=0 and missionname in (select mission from cre_par where cr_or_pr=0 and user in (select user2 from is_friend where user1='$name')) order by missiontime desc) as tmp, user) where user.username=tmp.user",$id);
