...
import com.example.helpandgetfun.viewcontroller.*;
...
public class FragmentOtherPage extends Fragment implements OnRefreshListener<ListView>{
	...
	public void onActivityCreated(Bundle savedInstanceState) {
		...
		DataUtils.otherList = new ArrayList<Map<String, Object>>();
		adapter = new MyAdapter(getActivity().getApplicationContext(), DataUtils.otherList, R.layout.pulldown_item,
	...
	private class GetDataTask extends AsyncTask<Void, Void, List<Map<String, Object> > > {
		...
		protected void onPostExecute(List<Map<String, Object> > result) {
			if (result != null) {
				DataUtils.otherList.clear();
				if (result.size() != 0) {
					DataUtils.otherList.addAll(result);
				}
				adapter.notifyDataSetChanged();
	...
	private class GetMoreTask extends AsyncTask<Void, Void, String[] > {
		...
		protected void onPostExecute(String[] result) {
			if (DataUtils.otherList.size() > adapter.getCount()) {
	    		if (DataUtils.otherList.size() >= adapter.getCount() + 10)
	    			adapter.setCount(adapter.getCount() + 10);
	    		else
	    			adapter.setCount(DataUtils.otherList.size());
		    	adapter.notifyDataSetChanged();
	    	}
			mListView.onRefreshComplete();
			isRefreshing = false;
