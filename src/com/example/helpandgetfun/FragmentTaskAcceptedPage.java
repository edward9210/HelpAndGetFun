...
public class FragmentTaskAcceptedPage extends Fragment implements OnRefreshListener<ListView>{
	...
	public void onActivityCreated(Bundle savedInstanceState) {
		...
		DataModel.taskAcceptedList.clear();
		if (adapter != null)
	...
	private class GetDataTask extends AsyncTask<Void, Void, List<Map<String, Object> > > {
		...
		protected void onPostExecute(List<Map<String, Object> > result) {
			if (result != null) {
				DataModel.taskAcceptedList.clear();
				if (result.size() != 0) {
					DataModel.taskAcceptedList.addAll(result);
				}
				adapter.notifyDataSetChanged();
		...
		protected List<Map<String, Object> > doInBackground(Void... params) {
			...
			try {
				return DataModel.getTaskAcceptedData();
	...
	private class GetMoreTask extends AsyncTask<Void, Void, String[] > {
		...
		protected void onPostExecute(String[] result) {
			if (DataModel.taskAcceptedList.size() > adapter.getCount()) {
	    		if (DataModel.taskAcceptedList.size() >= adapter.getCount() + 10)
	    			adapter.setCount(adapter.getCount() + 10);
	    		else
	    			adapter.setCount(DataModel.taskAcceptedList.size());
		    	adapter.notifyDataSetChanged();
	    	}
			mListView.onRefreshComplete();
			isRefreshing = false;
