...
public class InfoCenterActivity extends Activity{
	...
	private void setAllWidget() {
		...
		if (DataModel.infoList == null)
			DataModel.infoList = new ArrayList<Map<String, Object>>();
		
		adapter = new MyAdapter(this, DataModel.infoList , R.layout.info_item,
	...
	private class GetDataTask extends AsyncTask<Void, Void, List<Map<String, Object> > > {
		...
		protected void onPostExecute(List<Map<String, Object> > result) {
			if (result != null) {
				...
				if (result.size() != 0) {
					mMoreBnt.setText("更多");
					DataModel.infoList.addAll(result);
				}
	...
	private class SendDataTask extends AsyncTask<String, Void, String> {
		...
		protected void onPostExecute(String result) {
			if (result.equals(DataModel.SENDMES_SUCCESS))
		...
		protected String doInBackground(String... params) {
			return DataModel.sendMes(senderName, content);
		}
	}
	...
	private Handler mUIHandler = new Handler(){
		...
		public void handleMessage(Message msg) {
		    switch (msg.what) {
		    case MORE_FINISHED:
		    	if (DataModel.infoList.size() > adapter.getCount()) {
		    		if (DataModel.infoList.size() >= adapter.getCount() + 10)
		    			adapter.setCount(adapter.getCount() + 10);
		    		else
		    			adapter.setCount(DataModel.infoList.size());
			    	adapter.notifyDataSetChanged();
		    	}
				mMoreBnt.setText("更多");
		    	break;
